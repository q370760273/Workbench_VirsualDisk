...
public class RedependenceInfoDrawer : GuiDrawer
{
    private int _showCount;
    private List<AssetData> _showFiles = new List<AssetData>();

    public RedependenceInfoDrawer(GuiView view) : base(view)
    {
    }
    ...
    private void DrawFoldout(AssetFile file)
    {
        ...
        {
            _showCount += file.reDefFiles.Count;
            EditorGUI.indentLevel++;
            foreach (AssetFile child in file.reDefFiles)
            {
                if (CheckLoop(child))
                {
